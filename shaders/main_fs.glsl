...
// Simplified - no parallax mapping, just return original coordinates
...
void main() {
  // Sample from texture atlas using original UV coordinates
  vec3 base_texture_colour = vec3(texture(u_texture, fs_textureCoord));
  
  // Flat, even lighting - just use the texture color or diffuse color directly
  vec3 compoundDiffuseColour = diffuse.texture * base_texture_colour + (1.0 - diffuse.texture) * diffuse.color;

  // Output the color directly without any lighting calculations
  outColor = vec4(compoundDiffuseColour, 1.0);
}
