...
class Piece {
  constructor(id, vertices, normals, indices, textures, tangents, bitangents, faceIDs) {
    this.id = id;
    ...
    this.faceIDs = faceIDs;  // Array of face IDs for each vertex
...
/**
 * Calculate face ID for each vertex based on its normal
 * Face IDs: 0=Front(+Z), 1=Back(-Z), 2=Left(-X), 3=Right(+X), 4=Up(+Y), 5=Down(-Y)
 * @param {number[]} normals - Array of vertex normals [x1,y1,z1, x2,y2,z2, ...]
 * @return {number[]} Array of face IDs, one per vertex
 */
function calculateFaceIDs(normals) {
  const faceIDs = [];
  const threshold = 0.5; // Threshold for determining dominant axis
  ...
  for (let i = 0; i < normals.length; i += 3) {
    ...
    if (absZ >= absX && absZ >= absY) {
      // Z axis is dominant
      faceID = nz > 0 ? 0 : 1; // Front : Back
    } else if (absX >= absY && absX >= absZ) {
      // X axis is dominant
      faceID = nx > 0 ? 3 : 2; // Right : Left
    } else {
      // Y axis is dominant
      faceID = ny > 0 ? 4 : 5; // Up : Down
    }
...
async function initializePieces(meshDir) {
  ...
  for (let id = 0; id < 26; id++) {
    ...
    // Calculate face IDs based on normals
    const faceIDs = calculateFaceIDs(meshObj.vertexNormals);
