...
export function CubeRenderer(cube, gl, program, textureDir) {
  this.init = function () {
    ...
    const faceIDAttributeLocation = gl.getAttribLocation(program, "a_faceID");
    
    console.log("faceIDAttributeLocation:", faceIDAttributeLocation);
    ...
    for (let i = 0; i < 26; i++) {
      ...
      let faceIDs = cube.pieceArray[i].faceIDs;
      ...
      // Add face ID buffer
      if (faceIDAttributeLocation >= 0 && faceIDs) {
        const faceIDBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, faceIDBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(faceIDs), gl.STATIC_DRAW);
        gl.enableVertexAttribArray(faceIDAttributeLocation);
        gl.vertexAttribPointer(faceIDAttributeLocation, 1, gl.FLOAT, false, 0, 0);
      } else {
        console.warn("faceID attribute not found or faceIDs not available");
      }
  ...
  this.loadTexture = async function () {
    setLoadingInfo("texture", "Loading texture atlas");
    
    const image = new Image();
    image.src = `${textureDir}CubeTexture.png`;
    await image.decode();
  ...
  this.renderCube = function (matrices) {
    gl.uniform1i(this.textureLocation, TEXTURE_OFFSET);
    gl.uniformMatrix4fv(
